...
def dim3_scatter_plot(series1, series2, initial_state, global_best_state, opt_test=0):
    '''
    ...
    ax.scatter(initial_state[0], initial_state[1], initial_state[2], color='r', marker='v')
    ax.scatter(global_best_state[0], global_best_state[1], global_best_state[2], color='k', marker='v', s=20)
    ...
    if not opt_test:
        ...
        ax.set_xlim(initial_state[0] - pos_limits, initial_state[0] + pos_limits)
        ax.set_ylim(initial_state[1] - pos_limits, initial_state[1] + pos_limits)
        ax.set_zlim(initial_state[2] - pos_limits, initial_state[2] + pos_limits)
    ...
    ax.scatter(initial_state[3], initial_state[4], initial_state[5], color='r', marker='v')
    ...
    if not opt_test:
        ...
        ax.set_xlim(initial_state[3] - vel_limits, initial_state[3] + vel_limits)
        ax.set_ylim(initial_state[4] - vel_limits, initial_state[4] + vel_limits)
        ax.set_zlim(initial_state[5] - vel_limits, initial_state[5] + vel_limits)
    plt.legend(loc='upper left')
...
def plot_propagated_trajectories(original_trajectory, propagated_trajectory, original_initial, propagated_initial):
    '''
    ...
    ax.scatter(original_initial[0], original_initial[1], original_initial[2], color='g', marker='v')
    ax.scatter(propagated_initial[0], propagated_initial[1], propagated_initial[2], color='b', marker='v')
...
def present_results(initial, propagated):
    ...
    result_matrix = np.array([initial_vect, propagated_vect, diff_vect, dist_vect])
