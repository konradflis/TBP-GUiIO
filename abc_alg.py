...
import random
from data_load import transfer_raw_data_to_trajectory, convert_to_metric_units
...
class Swarm:
    ...
    def __init__(self, population_size, number_of_measurements, max_iterations, chosen_states, initial_state):
...
class Food:
    ...
    def propagate(self, time_vect, time_span):
        ...
        propagated_trajectory = integrate.solve_ivp(self.__diff_equation, [0, time_span],
                                                    [x, y, z, vx, vy, vz], t_eval=time_vect)
    ...
    def __diff_equation(self, t, parameters):
        ...
        out_x1, out_x2 = array([x2, 2 * y2 + x1 - (1 - self.mu) * (x1 + self.mu) / r_se - self.mu * (x1 + self.mu - 1) / r_sm])
        out_y1, out_y2 = array([y2, -2 * x2 + y1 - (1 - self.mu) * y1 / r_se - self.mu * y1 / r_sm])
        out_z1, out_z2 = array([z2, -1 * (1 - self.mu) * z1 / r_se - self.mu * z1 / r_sm])

        return [out_x1, out_y1, out_z1, out_x2, out_y2, out_z2]

    def generate_neighbours(self, number_of_neighbours, neighbours_pos_limits, neighbours_vel_limits, neighbourhood_type=0, dim_probability=0.5):
        """
        ...
        if neighbourhood_type == 0:
            for _ in range(number_of_neighbours):
                ...
                for dim in range(3):
                    new_state[dim] = self.state[dim] + random.uniform(-1 * pos_limits, pos_limits)
                for dim in range(3, 6):
                    new_state[dim] = self.state[dim] + random.uniform(-1 * vel_limits, vel_limits)
        ...
        if neighbourhood_type == 1:
            accepted_dims = [dim for dim in range(6) if random.random() < dim_probability]
            for _ in range(number_of_neighbours):
                new_state = self.state
                for dim in accepted_dims:
                    if dim < 3:
                        new_state[dim] += random.uniform(-1 * pos_limits, pos_limits)
                    elif dim >= 3:
                        new_state[dim] += random.uniform(-1 * vel_limits, vel_limits)
    ...
    def choose_best_neighbour(self):
        ...
        source_all_scores = [new_source.score for new_source in source_all_options]
    ...
    def calculate_cost(self):
        ...
        if propagated_trajectory.shape[1] == self.swarm.number_of_measurements:
            for it in range(self.swarm.number_of_measurements):
                error += linalg.norm(original_trajectory[it, :3] - propagated_trajectory[:3, it])
            self.score = error
        else:
            self.score = np.inf
    ...
    def scout_new_source(self, initial_state, generating_method=0, vel_limits=0.05):
        '''
        ...
        if generating_method == 0:
            ...
            new_state = [np.random.uniform(-1 * pos_limits, pos_limits), np.random.uniform(-1 * pos_limits, pos_limits),
            np.random.uniform(-1 * pos_limits, pos_limits), np.random.uniform(-1 * vel_limits, 1 * vel_limits),
            np.random.uniform(-1 * vel_limits, 1 * vel_limits), np.random.uniform(-1 * vel_limits, 1 * vel_limits)] + initial_state
            ...
            random_vel_factor = [1 + np.random.uniform(-1 * vel_limits, 1 * vel_limits) for _ in range(3)]
            new_state = np.array([np.random.uniform(-1 * pos_limits, pos_limits) + self.swarm.global_best_state[0], np.random.uniform(-1 * pos_limits, pos_limits) + self.swarm.global_best_state[1],
            np.random.uniform(-1 * pos_limits, pos_limits) + self.swarm.global_best_state[2], random_vel_factor[0] * self.swarm.global_best_state[3],
            random_vel_factor[1] * self.swarm.global_best_state[4], random_vel_factor[2] * self.swarm.global_best_state[5]])

        elif generating_method == 2:
            """Alternative 2: new_state depending on initial state (position) and global best state (velocity)"""
            random_vel_factor = [1 + np.random.uniform(-1 * vel_limits, 1 * vel_limits) for _ in range(3)]
            new_state = np.array([np.random.uniform(-1 * pos_limits, pos_limits) + initial_state[0], np.random.uniform(-1 * pos_limits, pos_limits) + initial_state[1],
            np.random.uniform(-1 * pos_limits, pos_limits) + initial_state[2], random_vel_factor[0] * self.swarm.global_best_state[3],
            random_vel_factor[1] * self.swarm.global_best_state[4], random_vel_factor[2] * self.swarm.global_best_state[5]])
...
def final_plot(initial_swarm, final_swarm, chosen_states, initial_state, max_iterations, best_scores_vector):
    '''
    ...
    return original_trajectory, propagated_trajectory, initial_state[:3] * 389703, final_swarm.global_best_state[:3] * 389703, series1, series2

def abc_alg(max_iterations, population_size, employee_phase_neighbours, onlooker_phase_neighbours, number_of_measurements,
        neighbours_pos_limits, neighbours_vel_limits, inactive_cycles_limit, inactive_cycles_setter=0,
        probability_distribution_setter=0, generating_method=0, neighbourhood_type=0, neigh_percent=0.02,
        dim_probability=0.5, orbit_filepath="L2_7days.txt"):
    '''
    ...
    :param inactive_cycles_setter: optional - modificates the number of inactive cycles depending on the current iteration
    ...
    swarm = Swarm(population_size, number_of_measurements, max_iterations, chosen_states, initial_state)
    ...
    for it in range(max_iterations):
        ...
        for source in swarm.sources:
            ...
            source.generate_neighbours(employee_phase_neighbours, neighbours_pos_limits, neighbours_vel_limits, neighbourhood_type, dim_probability)
        ...
        if probability_distribution_setter == 0:
            ...
            total_cost = np.sum([range_score - source.score for source in swarm.sources])
            for source in swarm.sources:
                source.probability = (range_score - source.score) / total_cost
                probabilities_vector.append(probabilities_vector[-1] + source.probability)
        ...
        for _ in range(swarm.population_size):
            ...
            for idx, border_prob in enumerate(probabilities_vector, -1):
                if border_prob > probability:
                    ...
                    break
            swarm.sources[chosen_state_idx].generate_neighbours(onlooker_phase_neighbours, neighbours_pos_limits, neighbours_vel_limits, neighbourhood_type, dim_probability)
        ...
        for source in swarm.sources:
            if source.active_flag is False:
                ...
                if source.inactive_cycles >= inactive_cycles_limit and it >= 1:
                    source.scout_new_source(initial_state, generating_method, neigh_percent)
        ...
        iteration_best_score_idx = np.argmin([source.score for source in swarm.sources])
    ...
    return [initial_swarm, final_swarm, chosen_states, initial_state, max_iterations, best_scores_vector]
...
def main():
    ...
    abc_alg(max_iterations, population_size, employee_phase_neighbours, onlooker_phase_neighbours, number_of_measurements,
        neighbours_pos_limits, neighbours_vel_limits, inactive_cycles_limit, inactive_cycles_setter=0,
        probability_distribution_setter=0, generating_method=0, neighbourhood_type=0, neigh_percent=0.02,
        dim_probability=0.5)
